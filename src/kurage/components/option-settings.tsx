...
export const SettingsForm = ({ settings, onCompleted }) =>
{
    ...
    const updateOptions = useCallback(async () =>
    {
        if(!isLoading)
        {
            ...
            try
            {
                ...
                createSuccessNotice(__('updated', 'md-table-editor'), { type: 'snackbar'})
                onCompleted(true)
            }
            catch(e)
            {
    ...
    return (
        <div>
            ...
            <Button disabled={isLoading} variant="primary" style={{width: '100%'}} onClick={() => updateOptions()}>{ __('Update', 'md-table-editor')}</Button>

            <p>{ __('This will update the options used globally, but you will need to reload the page for the changes to take effect.', 'md-table-editor') }</p>
        </div>
    )
}
